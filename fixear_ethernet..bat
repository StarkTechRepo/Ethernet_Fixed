...
REM -- Fixeando problemas de Ethernet --
echo.
echo === Deshabilitando adaptadores de red ===
echo.
wmic path win32_networkadapter where "NetConnectionID Like 'Ethernet%' AND NetEnabled='True'" call disable
timeout /t 3 >nul

echo.
echo === Habilitando adaptadores de red ===
echo.
wmic path win32_networkadapter where "NetConnectionID Like 'Ethernet%' AND NetEnabled='False'" call enable
timeout /t 3 >nul
...
echo.
echo Fixeo de Ethernet completado. ¡Disfruta de una conexión estable!
pause
